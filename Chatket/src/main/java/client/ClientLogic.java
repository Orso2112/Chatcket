...
public class ClientLogic {
    ...
    private final Map<String, MessageListener> chatListeners = new ConcurrentHashMap<String, MessageListener>();
    private final ExecutorService executor = Executors.newCachedThreadPool();
    private final List<PrintWriter> p2pConnectedClients = new CopyOnWriteArrayList<PrintWriter>();
    private final Map<String, Color> userColors = new ConcurrentHashMap<String, Color>();
    ...
    public Color getUserColor(String username) {
        if (username == null) return PALETTE[0];
        Color c = userColors.get(username);
        if (c != null) return c;
        int idx = Math.abs(username.hashCode()) % PALETTE.length;
        Color chosen = PALETTE[idx];
        userColors.put(username, chosen);
        return chosen;
    }
}