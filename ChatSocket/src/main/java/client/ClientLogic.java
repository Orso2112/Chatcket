...
public class ClientLogic {
    ...
    private String encrypt(String plainText, String key64) {
        if (key64 == null) {
            System.err.println("Errore: Chiave di crittografia nulla.");
            return null;
        }
        try {
            ...
            System.err.println("Errore nella crittografia: " + e.getMessage());
            return null;
        }
    }

    private String decrypt(String encryptedText, String key64) {
        if (key64 == null) {
            System.err.println("Errore: Chiave di decrittografia nulla.");
            return null;
        }
        try {
            byte[] key = Base64.getDecoder().decode(key64);
    ...
    public boolean joinChat(String ip, String passKey, MainFrame mainFrame, String chatName) {
        ...
        try {
            Socket chatSocket = new Socket(ip, 8000);
            System.out.println("Connesso al moderatore su " + ip + ":8000");
            ...
            String ownKey64 = deriveKey(this.username);
            if (ownKey64 == null) {
                System.err.println("Impossibile derivare la chiave per l'utente: " + this.username);
                chatSocket.close();
                return false;
            }
            String encryptedPassKey = encrypt(passKey, ownKey64);
            if (encryptedPassKey == null) {
                System.err.println("Impossibile crittografare la passKey.");
                chatSocket.close();
                return false;
            }

            outToModerator.println("PASSKEY_REQUEST:" + encryptedPassKey);

            String response = inFromModerator.readLine();
            System.out.println("Risposta dal moderatore: " + response);
    ...
    private class ChatClientHandler implements Runnable {
        ...
        public void run() {
            try (BufferedReader inFromClient = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
                ...
                if (requestLine != null) {
                    if (requestLine.startsWith("PASSKEY_REQUEST:")) {
                        String encryptedPassKey = requestLine.substring("PASSKEY_REQUEST:".length());

                        if (this.moderatorUsername == null) {
                            System.out.println("Errore: Username del moderatore insesistente");
                            outToClient.println("false");
                            return;
                        }

                        String moderatorKey64 = deriveKey(moderatorUsername);
                        if (moderatorKey64 == null) {
                            System.out.println("Errore: Impossibile derivare la chiave del moderatore.");
                            outToClient.println("false");
                            return;
                        }
                        String doubleEncryptedPassKey = encrypt(encryptedPassKey, moderatorKey64);
                        if (doubleEncryptedPassKey == null) {
                            System.out.println("Errore: Impossibile crittografare la passKey del client.");
                            outToClient.println("false");
                            return;
                        }

                        outToClient.println(doubleEncryptedPassKey);

                        String passKeyWithModeratorHash = inFromClient.readLine();
                        if (passKeyWithModeratorHash == null) {
                            System.out.println("Errore: Nessuna passKey ricevuta dal client dopo il doppio hash.");
                            outToClient.println("false");
                            return;
                        }

                        String decryptedPassKey = decrypt(passKeyWithModeratorHash, moderatorKey64);
                        if (decryptedPassKey == null) {
                            System.out.println("Errore: Impossibile decrittografare la passKey del client.");
                            outToClient.println("false");
                            return;
                        }
                        ...
                        boolean isValid = (configFileName == null) || (storedPassKey != null && storedPassKey.equals(decryptedPassKey));
                    ...
                    System.out.println("Connessione ricevuta ma nessun messaggio inviato immediatamente.");
                }
